...
void ReversePurePursuit::onTimer()
{
  ...
  // --- 符号の反転を削除した(2026-09-06 ユーザー指摘) ---
  //
  // 【二重反転になっていた】後退の pure pursuit の正しい式は2通りあり、
  // どちらか一方だけを使う。
  //   A: 車体**後方**から測った角度 beta を使う -> 反転が**必要**
  //   B: 車体**前方**から測った角度 alpha を使う -> 反転は**不要**
  // sin(alpha) = sin(pi + beta) = -sin(beta) なので、alpha を使えば
  // 符号は自動的に正しくなる。
  //
  // ここは上で alpha(前方基準)を計算しているのに、A の反転も掛けていた。
  // その結果 **常に理想の逆へ舵を切っていた**
  // (ユーザー報告「毎回、理想の逆にハンドルを回している気がします」)。
  //
  // 【数値での確認】原点 yaw=0、L=2、目標距離 1.5m、目標が後方かつ左の
  // (-2, +0.5) の場合:
  //   必要な舵: delta=+0.3(左)で後退すると後軸は (-0.50, +0.02) へ動く -> 正が必要
  //   反転あり: alpha=2.897, sin=+0.2425 -> +0.574 -> 反転して **-0.574(右)** = 逆
  //   反転なし: **+0.574(左)** = 正しい
  //
  // 【なぜ今まで露見しなかったか】後退の加速度の符号が逆で常にブレーキだったため、
  // 車がほとんど後退せず、舵の向きを観測できていなかった。
  // 加速度を直したら「経路で動かない」が 13回 -> 2回 に減り、
  // 代わりに舵の向きの誤りが表に出た。
